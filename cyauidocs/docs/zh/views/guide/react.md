 ...
 * @LastEditors: Chengya
 * @LastEditTime: 2025-09-11 16:54:03
-->
...
```bash
npx create-react-app my-app // 或者全局安装 create-react-app my-app; npm install -g create-react-app
然后使用 create-react-app my-app初始化项目
cd my-app
npm start
```

#### 三、 通过 npm/yarn 安装并通过模块导入的方式使用

这是现代 React 项目的主流方式，适用于使用打包工具（Webpack、Vite、Parcel 等）来对资源进行管理，构建的项目

1. 通过 npm init -y 初始化项目

2. 配置 react 开发环境 安装 webpack webpack-cli webpack-dev-server html-webpack-plugin;npm i webpack webpack-cli webpack-dev-server html-webpack-plugin -D (开发依赖)

3. 根目录下创建 src 目录，在 src 创建 index.html 和 index.js

4. 根目录创建 webpack.config.js 文件，添加打包配置

5. 安装 react 相关依赖 react react-dom; npm install react react-dom -S（生产依赖）
